...
class Parameters:
    ...
    def __init__(
        ...
        columns: int,
        rows: int,
        vertical_video_columns: int,
        vertical_video_rows: int,
        spacing: int,
        ...
        timestamp_font_name: str,
        timestamp_font_size: int,
        timestamp_font_color: str,
        timestamp_shadow_color: str,
        comment_label: str,
        comment_text: str,
        skip_seconds: float,
        suffix: str,
...
class VideoThumbnailer:
    ...
    def __init__(self, parameters=None):
        ...
        if self.parameters.output_directory is not None:
            ...
            if not self.parameters.output_directory.exists():
                try:
                    ...
                    message = "Unable to create output directory '{}': {}".format(
                        self.parameters.output_directory.absolute(), e
                    )
            ...
            # Exit if the path of the directory already exists, but is not a directory
                ...
                message = "Path of the output directory already exists and is not a directory: '{}'".format(
                    self.parameters.output_directory.absolute()
                )
                raise VideoThumbnailerException(message)
            # Exit if permission for accessing the output directory is not granted
                ...
                message = (
                    "Missing permission to access the output directory: '{}'".format(
                        self.parameters.output_directory.absolute()
                    )
                )
                raise VideoThumbnailerException(message)
            # Exit if permissions for writing to the output directory are not granted
                ...
                message = (
                    "Missing permission to write the output directory: '{}'".format(
                        self.parameters.output_directory.absolute()
                    )
                )
                raise VideoThumbnailerException(message)
        ...
        if not self.parameters.no_header:
            if self.parameters.header_font_name is None:
                ...
                self.__header_font = ImageFont.truetype(
                    font=self.parameters.header_font_name,
                    size=self.parameters.header_font_size,
                )
    ...
    def __parse_args() -> Namespace:
        parser = ArgumentParser(
            description="Python Video Thumbnailer. Command line tool for creating video preview thumbnails."
        )
        parser.add_argument(
            "--width",
            type=int,
        ...
        parser.add_argument(
            "--columns", type=int, help="The number of preview thumbnail columns."
        )
        parser.add_argument(
            "--rows", type=int, help="The number of preview thumbnail rows."
        )
        parser.add_argument(
            "--vertical-video-columns",
            type=int,
        ...
        parser.add_argument(
            "--no-header",
            action=VideoThumbnailer.__get_bool_store_action(
                not Parameters.DEFAULT_NO_HEADER
            ),
        ...
        parser.add_argument(
            "--override-existing",
            action=VideoThumbnailer.__get_bool_store_action(
                not Parameters.DEFAULT_OVERRIDE_EXISTING
            ),
        ...
        parser.add_argument(
            "--recursive",
            action=VideoThumbnailer.__get_bool_store_action(
                not Parameters.DEFAULT_RECURSIVE
            ),
        ...
        parser.add_argument(
            "--raise-errors",
            action=VideoThumbnailer.__get_bool_store_action(
                not Parameters.DEFAULT_RAISE_ERRORS
            ),
        ...
        parser.add_argument(
            "--verbose",
            action=VideoThumbnailer.__get_bool_store_action(
                not Parameters.DEFAULT_VERBOSE
            ),
    ...
    def __init_parameters(self):
        ...
        if config_file.is_file():
            ...
            if ConfigFile.CONFIG_SECTION_LAYOUT in config:
                ...
                if "width" in layout_options:
                    self.parameters.width = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "width"
                    )
                if "columns" in layout_options:
                    self.parameters.columns = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "columns"
                    )
                if "rows" in layout_options:
                    self.parameters.rows = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "rows"
                    )
                if "vertical_video_columns" in layout_options:
                    self.parameters.vertical_video_columns = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "vertical_video_columns"
                    )
                if "vertical_video_rows" in layout_options:
                    self.parameters.vertical_video_rows = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "vertical_video_rows"
                    )
                if "spacing" in layout_options:
                    self.parameters.spacing = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "spacing"
                    )
                if "background_color" in layout_options:
                    color_value = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "background_color"
                    )
                    if color_value is not None and color_value != "":
                        self.parameters.background_color = ImageColor.getrgb(
                            color_value
                        )
                if "no_header" in layout_options:
                    self.parameters.no_header = config.getboolean(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "no_header"
                    )
                if "header_font" in layout_options:
                    self.parameters.header_font_name = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "header_font"
                    )
                if "header_font_size" in layout_options:
                    self.parameters.header_font_size = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "header_font_size"
                    )
                if "header_font_color" in layout_options:
                    color_value = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "header_font_color"
                    )
                    if color_value is not None and color_value != "":
                        self.parameters.header_font_color = ImageColor.getrgb(
                            color_value
                        )
                if "timestamp_font" in layout_options:
                    self.parameters.timestamp_font_name = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "timestamp_font"
                    )
                if "timestamp_font_size" in layout_options:
                    self.parameters.timestamp_font_size = config.getint(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "timestamp_font_size"
                    )
                if "timestamp_font_color" in layout_options:
                    color_value = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "timestamp_font_color"
                    )
                    if color_value is not None and color_value != "":
                        self.parameters.timestamp_font_color = ImageColor.getrgb(
                            color_value
                        )
                if "timestamp_shadow_color" in layout_options:
                    color_value = config.get(
                        ConfigFile.CONFIG_SECTION_LAYOUT, "timestamp_shadow_color"
                    )
                    if color_value is not None and color_value != "":
                        self.parameters.timestamp_shadow_color = ImageColor.getrgb(
                            color_value
                        )
                    else:
            ...
            if ConfigFile.CONFIG_SECTION_HEADER in config:
                ...
                if "comment_label" in header_options:
                    self.parameters.comment_label = config.get(
                        ConfigFile.CONFIG_SECTION_HEADER, "comment_label"
                    )
                if "comment_text" in header_options:
                    self.parameters.comment_text = config.get(
                        ConfigFile.CONFIG_SECTION_HEADER, "comment_text"
                    )
            if ConfigFile.CONFIG_SECTION_VIDEO in config:
                ...
                if "skip_seconds" in video_options:
                    self.parameters.skip_seconds = config.getfloat(
                        ConfigFile.CONFIG_SECTION_VIDEO, "skip_seconds"
                    )
            if ConfigFile.CONFIG_SECTION_FILE in config:
                ...
                if "recursive" in file_options:
                    self.parameters.recursive = config.getboolean(
                        ConfigFile.CONFIG_SECTION_FILE, "recursive"
                    )
                if "suffix" in file_options:
                    self.parameters.suffix = config.get(
                        ConfigFile.CONFIG_SECTION_FILE, "suffix"
                    )
                if "jpeg_quality" in file_options:
                    self.parameters.jpeg_quality = config.getint(
                        ConfigFile.CONFIG_SECTION_FILE, "jpeg_quality"
                    )
                if "override_existing" in file_options:
                    self.parameters.override_existing = config.getboolean(
                        ConfigFile.CONFIG_SECTION_FILE, "override_existing"
                    )
                if "output_directory" in file_options:
                    output_directory = config.get(
                        ConfigFile.CONFIG_SECTION_FILE, "output_directory"
                    )
            ...
            if ConfigFile.CONFIG_SECTION_PROGRAM in config:
                ...
                if "raise_errors" in program_options:
                    self.parameters.raise_errors = config.getboolean(
                        ConfigFile.CONFIG_SECTION_PROGRAM, "raise_errors"
                    )
                if "verbose" in program_options:
                    self.parameters.verbose = config.getboolean(
                        ConfigFile.CONFIG_SECTION_PROGRAM, "verbose"
                    )
        ...
        if args.timestamp_font_color is not None:
            self.parameters.timestamp_font_color = ImageColor.getrgb(
                args.timestamp_font_color
            )
        if args.timestamp_shadow_color is not None:
            self.parameters.timestamp_shadow_color = ImageColor.getrgb(
                args.timestamp_shadow_color
            )
    ...
    def create_and_save_preview_thumbnails_for(self, file_path: Path) -> None:
        ...
        if self.parameters.output_directory is None:
            ...
            image_path = self.parameters.output_directory / "{}{}.jpg".format(
                file_path.name, self.parameters.suffix
            )

        if image_path.exists():
            if not self.parameters.override_existing:
                print(
                    "The path, where the preview image should be saved already exists, but shall not be overridden. Canceling creation of '{}'.".format(
                        image_path.absolute()
                    ),
                    file=sys.stderr,
                )
                ...
                print(
                    "The path, where the preview image should be saved already exists, but is not a file. Canceling creation of '{}'.".format(
                        image_path.absolute()
                    ),
                    file=sys.stderr,
                )
                ...
                print(
                    "The file '{}' already exists and will be overridden as requested.".format(
                        image_path.absolute()
                    )
                )
    ...
    def create_preview_thumbnails_for(self, file_path: Path) -> Image:
        ...
        # Check if the video codec is HEVC
        if (
            video_metadata
            and "format" in video_metadata
            and video_metadata["format"] == "HEVC"
        ):
            raise VideoThumbnailerException(
                "HEVC codec detected. Thumbnail generation is not supported for HEVC videos."
            )

        # General and video metadata information must be present
        if general_metadata is None:
            raise VideoThumbnailerException(
                "Could not read general metadata from '{}'.".format(file_path)
            )
        if video_metadata is None:
            raise VideoThumbnailerException(
                "Could not read video metadata from '{}'.".format(file_path)
            )
        ...
        if "rotation" in video_metadata.keys():
            ...
            if self.parameters.verbose:
                print(
                    "Video images are rotated by {}° according to metadata.".format(
                        video_rotation
                    )
                )
        ...
        try:
            ...
            raise VideoThumbnailerException(
                "Video stream in '{}' has no duration set in metadata. Cannot calculate preview timestamps.".format(
                    file_path
                )
            )
        ...
        if self.parameters.skip_seconds >= duration:
            print(
                "Time to skip at the beginning ({} s) is longer than the duration of the video ({} s)!".format(
                    self.parameters.skip_seconds, duration
                ),
                file=sys.stderr,
            )
            return
        ...
        if time_step < 1.0 / fps:
            print(
                "Video clip is too short to generate {} distinct preview thumbnails".format(
                    number_thumbnails
                ),
                file=sys.stderr,
            )
        ...
        size_info = None
        if file_size > 1024.0:
            size_info = "Size: {} B ({}), Duration: {}".format(
                file_size, Helper.format_size(file_size), Helper.format_time(duration)
            )
        else:
            size_info = "Size: {} B, Duration: {}".format(
                file_size, Helper.format_time(duration)
            )
        ...
        if not self.parameters.no_header:
            ...
            text_height_file_info = Helper.get_font_height(
                file_info, self.__header_font
            )
            text_height_size_info = Helper.get_font_height(
                size_info, self.__header_font
            )
            text_height_video_info = Helper.get_font_height(
                video_info, self.__header_font
            )
            text_height_audio_info = Helper.get_font_height(
                audio_info, self.__header_font
            )
            text_height_comment = 0
            if comment is not None:
                text_height_comment = Helper.get_font_height(
                    comment, self.__header_font
                )
        ...
        thumbnail_width = float(
            self.parameters.width - x_spacing * (columns + 1)
        ) / float(columns)
        ...
        if self.parameters.verbose:
            print(
                "Image dimensions: {} x {} -> {} x {} thumbnails with dimensions {} x {}".format(
                    image_width,
                    image_height,
                    columns,
                    rows,
        ...
        thumbnails_image = Image.new(
            "RGB", (image_width, image_height), color=self.parameters.background_color
        )
        ...
        if not self.parameters.no_header:
            x = x_spacing
            y = y_spacing
            thumbnails_draw.text(
                (x, y),
                file_info,
                self.parameters.header_font_color,
        ...
        if self.parameters.timestamp_font_name is None:
            ...
            timestamp_font = ImageFont.truetype(
                font=self.parameters.timestamp_font_name,
                size=self.parameters.timestamp_font_size,
            )
        ...
        for row_index in range(rows):
            y = (
                header_height
                + row_index * thumbnail_height
                + (row_index + 1) * y_spacing
            )
            for column_index in range(columns):
                # https://programtalk.com/vs2/python/8441/PyAV/tests/test_seek.py/
                target_timestamp = (
                    int(time / video_stream.time_base) + video_stream.start_time
                )
                ...
                timestamp_height = Helper.get_font_height(
                    formatted_time, timestamp_font
                )
                x_timestamp = (
                    x + thumbnail_width - timestamp_width - x_spacing_timestamp
                )
                y_timestamp = (
                    y
                    + thumbnail_height
                    - timestamp_height
                    - y_spacing_timestamp
                    - timestamp_shadow_offset
                )
                timestamp_position = (x_timestamp, y_timestamp)
                shadow_position = (
                    x_timestamp + timestamp_shadow_offset,
                    y_timestamp + timestamp_shadow_offset,
                )
                ...
                if self.parameters.timestamp_shadow_color is not None:
                    thumbnails_draw.text(
                        shadow_position,
                        formatted_time,
                        self.parameters.timestamp_shadow_color,
                        font=timestamp_font,
                    )
                ...
                thumbnails_draw.text(
                    timestamp_position,
                    formatted_time,
                    self.parameters.timestamp_font_color,
                    font=timestamp_font,
                )
                ...
                if self.parameters.verbose:
                    print(
                        "Captured preview thumbnail #{} of frame at {:.3f} s".format(
                            thumbnail_count, time
                        )
                    )
                time += time_step
    ...
    def process_file_or_directory(self, path: Path) -> None:
        ...
        if path.is_dir():
            if not os.access(path, os.X_OK):
                print(
                    "Cannot consider directory '{}'. Permission to access it is denied.".format(
                        path.absolute()
                    )
                )
                return
            if not os.access(path, os.R_OK):
                print(
                    "Cannot consider directory '{}'. Permission to list its files is denied.".format(
                        path.absolute()
                    )
                )
                return
            # If output directory is not specified otherwise, ensure write permission to this directory is granted
            if self.parameters.output_directory is None and not os.access(
                path, os.W_OK
            ):
                print(
                    "Cannot consider directory '{}'. Permission to write files to it is denied.".format(
                        path.absolute()
                    )
                )
                return
        ...
        # If the path is a file
            ...
            if not os.access(path, os.R_OK):
                print(
                    "Cannot consider video file '{}'. Permission to read it is denied.".format(
                        path.absolute()
                    )
                )
                return
            if self.parameters.output_directory is None and not os.access(
                path.parent, os.X_OK | os.W_OK
            ):
                print(
                    "Cannot consider video file '{}'. Permission to write files to its directory '{}' is denied.".format(
                        path.name, path.parent.absolute()
                    )
                )
                return
            ...
            print(
                "Path '{}' is neither a file nor a directory.".format(path.absolute())
            )
            return

...
class TikTokBaseIE(InfoExtractor):
    ...
    def _extract_web_formats(self, aweme_detail, webpage_url):
        COMMON_FORMAT_INFO = {
            'ext': 'mp4',
            'vcodec': 'h264',
            'acodec': 'aac',
        ...
        unique_formats = [{"width": w, "height": h} for w, h in unique_formats]
        response = self._download_json(
            f"https://www.tikwm.com/api/?url={webpage_url}&hd=1", "tikwm"
        )
        if response.get("msg") == "success":
            ...
            duration = traverse_obj(response, ("data", "duration"))

            for i, (q, size_key) in enumerate(sizes.items()):
                url = traverse_obj(response, ("data", q))
                if url and self._is_valid_url(url, "tikwm"):
                    size = traverse_obj(response, ("data", size_key)) or (
                        traverse_obj(response, ("data", "size")) + 15000
                    )
                    tbr = math.ceil((size * 8) / duration / 1000) if size and duration else None
                    format_index = min(i, len(unique_formats) - 1)
                    try:
                        selected_format = unique_formats[format_index]
                    except IndexError:
                        selected_format = {"width": None, "height": None}
    ...
    def _parse_aweme_video_web(self, aweme_detail, webpage_url, video_id, extract_flat=False):
        ...
        return {
            'id': video_id,
            'formats': None if extract_flat else self._extract_web_formats(aweme_detail, webpage_url),
...
class TikTokIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            return self._parse_aweme_video_web({}, url, video_id)
